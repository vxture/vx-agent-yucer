...
/**
 * 环形进度 - mockup 的 `.ring`（百分比读数嵌在圆环里，圆环按比例着色）。
 * DS 没有环形/仪表盘 Progress 变体，本地垫片见 TD-033
 * (docs/60-operations/00-index.md) - 恢复条件是 DS 补上这个变体后删掉本
 * 组件，改为消费该元件。只用 DS 语义色 token：底环固定 `stroke-border`
 * （中性，同 mockup 的 `--border`），值弧 `stroke-current` 配 tone 对应的
 * `text-*` 类（`TONE_SURFACE[tone]` 里的那一个），不引入任何硬编码色值。
 */
export function RingGauge({
  ...
  /** 0-100。裁到这个区间，一个越界的分数不应该画出一整圈以上的弧。 */
  ...
  const r = (size - 4.5) / 2;
  const c = 2 * Math.PI * r;
  const toneTextClass = TONE_SURFACE[tone].split(" ").find((cls) => cls.startsWith("text-"));
  return (
    <div className="relative flex-none" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90">
        ...
        />
        <circle
          className={`stroke-current ${toneTextClass ?? ""}`}
          fill="none"
          strokeWidth={4.5}
          strokeLinecap="round"
          strokeDasharray={`${c} ${c}`}
      ...
      <div className="font-display absolute inset-0 grid place-items-center text-[0.8125rem] font-extrabold tabular-nums">
        {Math.round(clamped)}
      </div>
    </div>
  );
