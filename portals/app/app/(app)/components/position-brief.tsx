...
export interface PositionBriefProps {
  ...
  readonly projects: readonly { id: string; name: string; health: string; href: string }[];
  ...
  readonly problems: readonly { id: string; claim: string; rule: string | null }[];
...
export function PositionBrief({
  ...
  return (
    <div className="flex flex-col gap-md">
      {/* ---- The other side ------------------------------------------------ */}
      <Card className="p-md">
        <SectionHeader level={3} title={POSITION_TEXT.external} description={POSITION_TEXT.externalWhy} />
        ...
        <div className="mt-md">
          ...
          <div className="mt-xs flex flex-wrap gap-lg">
            {chain.map((f) => (
              <div key={f.label}>
                <div className={["text-label-lg tabular-nums", f.tone ? TONE[f.tone] : "text-foreground"].join(" ")}>
                  {f.value}
                </div>
        ...
        <div className="border-border mt-md border-t pt-md">
          <p className="text-muted-foreground text-xs">{POSITION_TEXT.projects}</p>
          {projects.length === 0 ? (
            <p className="text-muted-foreground mt-xs text-body-sm">{POSITION_TEXT.noProjects}</p>
          ) : (
            <div className="mt-xs flex flex-wrap gap-xs">
              {projects.map((p) => (
                <Link key={p.id} href={p.href}>
                  <StatusBadge tone={p.health === "red" ? "danger" : p.health === "amber" ? "warning" : "success"}>
                    {p.name}
                  </StatusBadge>
                </Link>
              ))}
        ...
        <div className="border-border mt-md border-t pt-md">
          <p className="text-muted-foreground text-xs">{POSITION_TEXT.competition}</p>
          <p className="text-muted-foreground mt-xs text-body-sm">
            {rivalMentions.length > 0 ? POSITION_TEXT.competitionNone : POSITION_TEXT.competitionNoMention}
          </p>
          ...
          {rivalMentions.map((m) => (
            <blockquote key={m.id} className="border-warning/50 mt-sm max-w-[62ch] border-l-2 pl-sm">
              <cite className="text-muted-foreground block text-xs not-italic tabular-nums">{m.when}</cite>
      ...
      {/* ---- Our side ------------------------------------------------------ */}
      <Card className="p-md">
        <SectionHeader level={3} title={POSITION_TEXT.internal} description={POSITION_TEXT.internalWhy} />
        <div className="mt-md">
          <p className="text-muted-foreground text-xs">{POSITION_TEXT.problems}</p>
          <p className="text-muted-foreground mt-2xs text-xs">{POSITION_TEXT.problemsWhy}</p>
          {problems.length === 0 ? (
            <p className="text-muted-foreground mt-sm text-body-sm">{POSITION_TEXT.noProblems}</p>
          ) : (
            <div className="mt-sm flex flex-col gap-sm">
              {problems.map((p) => (
                <div key={p.id} className="bg-muted/40 border-border rounded-md border p-sm">
                  <p className="text-foreground max-w-[62ch] text-body-sm">{p.claim}</p>
      ...
      {/* ---- The plan ------------------------------------------------------ */}
      <Card className="p-md">
        <SectionHeader level={3} title={POSITION_TEXT.plan} description={POSITION_TEXT.planWhy} />
        {proposals.length === 0 ? (
          <p className="text-muted-foreground mt-md text-body-sm">{POSITION_TEXT.planEmpty}</p>
        ) : (
            ...
            {proposals.map((p) => (
              <div key={p.id} className="border-border flex flex-wrap items-start gap-md border-b py-sm last:border-b-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {p.rationale ? (
                    <p className="text-muted-foreground mt-2xs max-w-[62ch] text-xs">{p.rationale}</p>
                  ) : null}
                </div>
                {p.confidence !== null ? (
