...
export function memberOptions(
  ...
  /* THE HINT IS WHAT EVERY SHIPPED CARVE OF THIS FRAME SAYS about the member -
     五分法 中部 · 七分法 华中 for a province, 陕西三分法 关中 for a city. A
     hint, not a constraint: nothing refuses a selection that disagrees. */
  const carves = templatesFor(scope);
  return ground.map((m) => ({
    key: m.key,
    ...
    hint: carves
      .map((t) => {
        ...
        return d ? hint(templateName(t.key), d.name) : null;
      })
