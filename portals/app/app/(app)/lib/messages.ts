...
export const DEAL_SCORE_TEXT = {
  title: "商机评估分",
  why: "商机评估的五个维度，交易档案上的评估分和态势判决的五张卡都按它算：每维 0-100，按这里的权重加权平均。某一维没有任何可判断的信息时不计，权重按比例分给其余维度。",
  sum: (n: number) => `权重合计 ${n}/100`,
  weightsLabel: "维度权重",
  weightsHint: "五个权重合计必须是 100。某项设为 0 即不参与评分。",
  factor: {
    value: "需求价值",
    consensus: "买方共识",
    competition: "竞争位置",
  ...
  recentLabel: "新鲜天数",
  recentHint: "最近一次跟进在这么多天内，互动热度的「联系」为稳",
  quietLabel: "沉寂天数",
  quietHint: "超过这么多天没跟进为风险；两者之间为关注",
  save: "保存",
  reset: "恢复默认",
  formula: "评估分 = Σ(权重 × 维度分) ÷ Σ权重",
  saved: "评估分权重已保存",
  // 交易档案的评估分币
  coinLabel: "商机评估分",
  ...
  noConcern: "各维均满分",
  // 每张评估卡的一行: 差在哪、该做什么 (owner 2026-09-25: 找到工作差距，聚焦工作重点)
  gap: {
    pain: () => "补写痛点",
    metrics: () => "补量化价值",
    budget: () => "弄清预算",
    statusQuo: () => "有不作为信号，化解它",
    noEconomic: () => "找到经济决策人",
    reachEconomic: () => "约到经济决策人",
    coach: () => "发展一位内线",
    opposition: (n: number) => `处理 ${n} 位反对者`,
...
export const JUDGEMENT_ACTION_TEXT = {
  title: "分析与判断",
  // 研判与行动 (owner 2026-09-26): 待动手的事 + 分析与判断, one list.
  listTitle: "研判与行动",
  listSep: "、",
  // 每条的分析: 这一单自己的事实，不是规则的说明
  why: {
    pain: () => "痛点还没写明：客户为什么要改变，说不清",
    metrics: () => "量化价值还没写明：值多少，说不清",
    budget: () => "客户项目总投入还没填",
    statusQuo: (q: string) => `跟进里有不作为信号：「${q}」`,
    noEconomic: () => "本单还没有标注经济决策人",
    reachEconomic: (name: string, days: number | null) => (days === null ? `${name} 至今未联系` : `${name} 已 ${days} 天没接触`),
    coach: () => "本单还没有内线",
    opposition: (names: string) => `${names} 立场反对`,
    coverage: (n: number) => (n === 0 ? "本单还没有任何联系人" : `本单只接触了 ${n} 人`),
    decisionProcess: () => "客户的决策流程还没写明",
    rivalMentioned: (text: string) => `最近一次提到对手：「${text}」`,
    rivalUnknown: () => "跟进里没提到对手——不等于没有对手",
    neverTouched: () => "这一单还没有任何跟进记录",
    quiet: (days: number, date: string) => `最近一次跟进在 ${days} 天前（${date}）`,
    theirOverdue: (n: number, max: number) => `客户有 ${n} 件事逾期，最久 ${max} 天`,
    exitGap: (names: string) => `未满足：${names}`,
    stalled: (stage: string, days: number) => `${stage}已停 ${days} 天`,
    closePassed: (date: string) => `预计成交日 ${date} 已过`,
    slipped: (n: number) => `预计成交日已推迟 ${n} 次`,
    nextStep: () => "没有约好的下一步：双方都没有日期在未来的承诺",
    approval: (n: number) => `${n} 行报价低于底价，等待审批`,
  },
  evidence: "依据",
  toPlan: "加入计划",
  ask: "问参谋",
