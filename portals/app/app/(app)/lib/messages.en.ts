...
export const en: Dictionary = {
  ...
  DOMAIN_LABEL: {
    ...
    opportunityConfig: "Business settings",
    stage: "Pipeline stages",
    contracttype: "Contract types",
    businessform: "Business forms",
    industry: "Industries",
  ...
  RENEWAL_TEXT: {
    ...
    anchorContract: (no: string) => `by contract ${no} notice`,
    anchorProject: "by project end date",
    noticePassed: (days: number) => `notice passed ${days} days ago`,
    noticeIn: (days: number) => `${days} days to notice`,
    tagNoticeMissed: (n: number) => `${n} past notice`,
  ...
  WAR_ROOM_TEXT: {
    accountHealth: (n: number) => `Customer health ${n}`,
    accountSingleThread: "Single-threaded customer",
  ...
  CONTRACT_TYPE_TEXT: {
    ...
    why: "What kind of transaction a deal is - new logo / renewal / expansion. Rename, reorder, add or remove. A new deal is defaulted from the account's own history and a rep may change it.",
    add: "New type",
    save: "Save",
    codeLabel: "Type code",
    codeHint: "Cannot be changed after creation. An existing code means a rename.",
  ...
  BUSINESS_FORM_TEXT: {
    ...
    count: (n: number) => `${n} form${n === 1 ? "" : "s"}`,
    why: "What is being sold - bespoke project / standard product / consulting. Rename, reorder, add or remove. Stall days are set per form too.",
    add: "New form",
    save: "Save",
    codeLabel: "Form code",
    codeHint: "Cannot be changed after creation. An existing code means a rename.",
    ...
    colName: "Form",
    colFiled: "Opportunities",
    deleteConsequence: "The form is removed from the catalog. Opportunities already filed under it are unaffected - one still using it cannot be deleted.",
    colStallOverride: "Stall days",
    stallOverrideLabel: "Stall-days override",
    stallOverrideHint: "Leave blank to use the workspace's default stall days. A bespoke project usually negotiates longer than a standard product sale, which is why this sits on the form.",
  ...
  ROLE_TEXT: {
    ...
    membersTitle: (name: string) => `${name} · members`,
    membersWhy: (n: number) => `${n} members hold this role.`,
    membersEmptyWhy: "Nobody holds this role yet.",
    membersSearchPlaceholder: "Search member names",
    membersSearchEmpty: "No matching members",
    linkMembersTitle: (name: string) => `${name} · link members`,
    linkMembersWhy: "Check who should hold this role; saving takes effect immediately.",
    linkMembersCount: (n: number) => `${n} selected`,
    linkMembersDone: (n: number) => `Updated - ${n} now hold it`,
    linkMembersSave: "Save",
    ...
    toolbarCount: (n: number) => `${n} roles`,
    selectionNoun: "roles",
    clearSelection: "Clear selection",
    bulkRemove: "Delete",
    bulkRemoveTarget: (n: number) => ` the ${n} selected roles`,
    bulkRemoveConsequence: "The roles and their permission sets are deleted; this cannot be undone.",
    bulkRemoveDone: (removed: number) => `${removed} roles deleted`,
    bulkRemoveSkipped: (n: number) => `${n} still held by members, not deleted`,
    toolbarFilteredCount: (shown: number, total: number) => `${shown} of ${total} roles`,
    ...
    filterAllRanks: "All ranks",
    filterEmpty: "No matching roles",
  },
  ROW_OPS: {
  ...
  ORG_TEXT: {
    ...
    addMembersTitle: (name: string) => `${name} · add members`,
    addMembersWhy: "Check who should belong to this unit; saving takes effect immediately and does not affect their placement in other units.",
    addMembersCount: (n: number) => `${n} selected`,
    addMembersDone: (n: number) => `Updated - ${n} now belong here`,
    addMembersSave: "Save",
    remove: "Delete unit",
    removeTarget: (name: string) => `"${name}"`,
    ...
    formWhy: "Set this unit's parent, kind and leader.",
    formSectionWhy: "Parent unit, unit kind, code, unit name and leader.",
    ...
    discard: "Discard",
    saveFailed: "Save failed",
    formDivisionTitle: "Territories",
    divisionModeAggregate: "Aggregate downward",
    divisionModeInherited: "Inherit upward",
    divisionModeNone: "No division",
    formDivisionChoose: "Choose divisions",
    formDivisionDone: "Divisions updated.",
    formDivisionUnset: "Choose one",
    divisionSetLabel: (mode: string) => `Set: ${mode}`,
    divisionChosenLabel: "Chosen",
    formDivisionDrawerTitle: "Choose divisions",
    formDivisionDrawerWhy: "Tick the divisions this department belongs to directly; a division may have several departments. Ticking none leaves it to aggregate or inherit automatically from the org tree.",
    formDivisionDrawerEmpty: "No divisions to choose from yet. Create one under division setup first.",
    ...
    toolbarFilteredCount: (shown: number, total: number) => `${shown} of ${total} units`,
  ...
  ROLE_ERROR: {
    ...
    // Linking members goes through assignRole/revokeRole (authz/admin.ts) -
    // a different validation pass from the roles above, with its own codes.
    // unknown_role there is role_unknown here; sub_required should not
    // trigger in practice (sub always comes from an already-loaded member
    // row) but is kept so a raw code is never shown to the reader.
    unknown_role: "That role does not belong to this workspace",
    sub_required: "Missing member identifier",
  },
  TERRITORY_ERROR: {
    ...GATE_ERROR,
  ...
  PROPOSAL_ERROR: {
    abandoned_closed: "This deal was abandoned; a proposal cannot move it - reopen it first",
    exit_reason_required: "Losing a deal needs a reason the proposal does not carry - do it on the deal page",
    exit_reason_invalid: "The loss reason in this proposal does not apply",
    exit_note_required: "'Other' needs a sentence saying what it was",
    ...GATE_ERROR,
    province_unknown:
      "The province must be one of the 34 provincial-level divisions - pick one from the list",
    industry_unknown:
  ...
  OPPORTUNITY_ERROR: {
    ...
    abandoned_closed: "This deal was abandoned; tick reopen and give a reason before moving it",
    exit_reason_required: "Pick a reason",
    exit_reason_invalid: "That reason does not apply here",
    exit_note_required: "'Other' needs a sentence saying what it was",
    not_open: "Only an open deal can be abandoned",
    terminal_stage:
  ...
  OPPORTUNITY_TEXT: {
    ...
    advanceExitReason: "Why it was lost",
    advanceExitNote: "Details",
    advanceExitNoteRequired: "'Other' needs a sentence saying what it was",
    abandonOpen: "Abandon this deal",
    abandonTitle: "Abandon this deal",
    abandonHint:
      "Abandoning means we stop pursuing it. The stage stays where it is, and the deal leaves every forecast and total; it can be reopened later.",
    abandonReason: "Why we are giving it up",
    abandonSubmit: "Abandon",
    abandonCancel: "Cancel",
    abandonExempt: "A reason must be chosen before abandoning - that step is the confirmation",
    abandonedTitle: "Abandoned",
    abandonedDescription: "This deal was abandoned. To pursue it again, tick reopen and give a reason.",
    exitRecorded: (reason: string) => `Exit reason: ${reason}`,
  ...
  FORECAST_PARAM_TEXT: {
    ...
    stallHint:
      "This long at one stage and the suggestion drops a band. Not the same clock as 'nobody has talked to the customer' - that one is set separately, under Reminder thresholds. A business form can also set its own stall days, overriding this default.",
  },
  ...
  CONTRACT_TYPE_ERROR: {
    code_required: "A contract type needs a code",
    name_required: "A contract type needs a name",
    contract_type_in_use: "Opportunities are still filed under this contract type - move them first",
    move_at_edge: "Already at that end of the list",
    not_movable: "This one cannot be moved",
    not_found: "No such contract type - it may have just been deleted. Refresh and try again",

    ...GATE_ERROR,
  },

  BUSINESS_FORM_ERROR: {
    code_required: "A business form needs a code",
    name_required: "A business form needs a name",
    business_form_in_use: "Opportunities are still filed under this business form - move them first",
    move_at_edge: "Already at that end of the list",
    not_movable: "This one cannot be moved",
    stall_override_out_of_range: "A stall override runs from 1 to 365 days",
    not_found: "No such business form - it may have just been deleted. Refresh and try again",
  ...
  CUSTOMER_TYPE_ERROR: {
    ...GATE_ERROR,
    code_required: "A customer type needs a code",
    name_required: "A customer type needs a name",
    customer_type_in_use: "Customers are still filed under this type - move them first",
    move_at_edge: "Already at that end of the list",
    not_movable: "This one cannot be moved",
    not_found: "No such customer type - it may have just been deleted. Refresh and try again",
  },

  CUSTOMER_TYPE_TEXT: {
    configTitle: "Customer Type",
    noun: "type",
    configWhy: "How customers are filed for selling - direct, channel, reseller. One in use cannot be deleted.",
    add: "New type",
    edit: "Edit",
    save: "Save",
    code: "Type code",
  ...
  CUSTOMER_SIZE_ERROR: {
    ...GATE_ERROR,
    code_required: "A customer size needs a code",
    name_required: "A customer size needs a name",
    customer_size_in_use: "Customers are still filed under this size - move them first",
    move_at_edge: "Already at that end of the list",
    not_movable: "This one cannot be moved",
    not_found: "No such customer size - it may have just been deleted. Refresh and try again",
  },

  CUSTOMER_SIZE_TEXT: {
    configTitle: "Customer Size",
    noun: "size",
    configWhy: "The size tier a customer is filed under - enterprise, large, mid-market. Not headcount, which the customer reports - this is the tier the workspace uses to pick playbooks and approvers. One in use cannot be deleted.",
    add: "New size",
    edit: "Edit",
    save: "Save",
    code: "Size code",
  ...
  CUSTOMER_NATURE_ERROR: {
    ...GATE_ERROR,
    code_required: "A customer nature needs a code",
    name_required: "A customer nature needs a name",
    customer_nature_in_use: "Customers are still filed under this nature - move them first",
    move_at_edge: "Already at that end of the list",
    not_movable: "This one cannot be moved",
    not_found: "No such customer nature - it may have just been deleted. Refresh and try again",
  },

  CUSTOMER_NATURE_TEXT: {
    configTitle: "Customer Nature",
    noun: "nature",
    configWhy: "What kind of organisation a customer is - government, state-owned, private. Distinct from type (how it buys) and industry (what it does) - a government buyer's procurement process, budget cycle and qualification requirements are unrelated to either. One in use cannot be deleted.",
    add: "New nature",
    edit: "Edit",
    save: "Save",
    code: "Nature code",
  ...
  WINLOSS_TEXT: {
    ...
    dealReviewTitle: "Outcome and review",
    dealReviewOwed: "Review owed",
    dealReviewLocked: "Reviews need the Business plan",
    dealReviewEdit: "Edit the review",
    dealReviewExit: (reason: string) => `Exit reason: ${reason}`,
  ...
  POSITION_TEXT: {
    ...
    tierStandardDesc: "The default tier - changes no rules",
    tierKeyDesc: "Watch closely - no business plan needed yet",
    tierStrategicDesc: "Needs a business plan - the cadence rule reads it",
  ...
  ADMIN_TEXT: {
    ...
    entryHint: {
      ...
      opportunityConfig: "Contract types, business forms, pipeline stages, win/loss reasons, forecast bands, ageing bands, pricing currency - everything about how a deal moves, in one page",
      stage: "The stages a deal moves through - rename, reorder, default win rate, add/remove",
      contracttype: "What kind of transaction a deal is - new logo / renewal / expansion",
      businessform: "What is being sold - bespoke project / standard product / consulting",
      ...
      audit: "Who changed the configuration and when - member and role actions",
      ...
      diagnostics: "Whether the platform connection itself is healthy - identity, entitlement, usage",
    },
  ...
  CONTRACT_ERROR: {
    ...GATE_ERROR,
    contract_no_required: "A contract needs a number",
    contract_no_too_long: "Contract number is too long (64 characters at most)",
    contract_no_taken: "That contract number is already on file",
    name_required: "A contract needs a name",
    name_too_long: "Contract name is too long",
    unknown_status: "Unknown contract status",
    notice_out_of_range: "Notice period must be 0-365 whole days",
    amount_negative: "The amount cannot be negative",
    term_inverted: "The term ends before it starts",
    term_required:
      "An active contract needs a start and an end - without an end date no renewal window ever sees it",
    frozen_field: "Number, customer, source deal and a line's product are fixed once recorded",
    ...
    already_renewed: "This contract has already been renewed - a contract renews once",
    contract_not_renewable: "Only an active contract can renew; a draft has no renewal to record",
    renewal_before_original: "A renewal cannot start before the contract it renews",
    unknown_event_type: "Unknown renewal outcome",
    reason_required: "Say why - an outcome with no reason audits nothing",
    reason_too_long: "Reason is too long (255 characters at most)",
    invalid_date: "That date is not valid",
    not_found: "No such record, or it belongs to another workspace",
    unknown: "Contracts could not be read - try again shortly",
  },

  CONTRACT_TEXT: {
    tab: "Contracts",
    empty: "No contracts on file",
    readFailed: "Contracts could not be read - refresh shortly. This does not mean there are none.",
    add: "Record a contract",
    edit: "Edit",
    addLine: "Add line",
    removeLine: "Remove",
    ...
    drawerLine: "Contract line",
    save: "Save",
    saved: "Saved",
    removed: "Removed",
    fieldNo: "Contract number",
    fieldNoFrozen: "Fixed once recorded",
    fieldName: "Name",
    fieldStatus: "Status",
    fieldAmount: "Contract value",
    fieldCurrency: "Currency",
    fieldTermStart: "Starts",
    fieldTermEnd: "Ends",
    fieldNotice: "Notice (days)",
    fieldSigned: "Signed",
    fieldDeal: "Source deal",
    fieldDealNone: "None",
    fieldDealFrozen: "Fixed once recorded",
    fieldProduct: "Product",
    fieldQty: "Quantity",
    fieldUnitPrice: "Unit price",
    fieldLineEnd: "Line ends (blank = with the contract)",
    statusDraft: "Draft",
    statusActive: "Active",
    statusTerminated: "Terminated",
    phaseDraft: "Draft",
    phasePending: "Not started",
    phaseInForce: "In force",
    phaseLapsed: "Lapsed, not renewed",
    phaseTerminated: "Terminated",
    phaseRenewed: "Renewed",
    renew: "Renew",
    drawerRenew: "Renew into a new contract",
    renewHint: (no: string) =>
    ...
    outcomeLost: "Lost (not renewing)",
    outcomeDowngraded: "Renewed for less",
    fieldReason: "Reason",
    outcomeAppendOnly: "Renewal records are append-only - no edit or delete; record another to correct.",
    eventRenewed: (no: string) => `renewed as ${no}`,
    eventDowngraded: "renewed for less",
    eventLost: "lost",
    term: (start: string, end: string) => `${start} to ${end}`,
    termOpen: "No term yet",
    daysLeft: (n: number) => `ends in ${n} days`,
    noticeBy: (d: string) => `notify by ${d}`,
    lineTotal: "Lines total",
    noLines: "No lines yet",
    lineUntil: (d: string) => `until ${d}`,
    revenueTitle: "Installed revenue",
    revenueHint: "Annualized value scales each in-force contract to one year of its own term; the catalog does not mark products recurring, so this is not ARR",
    revenueAnnualized: (n: number) => `Annualized contract value (${n} in force)`,
    revenueLifetime: (n: number) => `Lifetime contract value (${n} signed)`,
    revenueUnpriced: (n: number) => `${n} more signed contract(s) have no amount and are not counted`,
    revenueNone: "No signed contract with an amount yet",
  ...
  ACCOUNT_TEXT: {
    ...
    collaboratorsLine: (names: readonly string[]) => `Collaborators ${names.join(", ")}`,
    dossier: "Dossier",
    dossierOwner: "Owner",
  ...
  DELIVERY_TEXT: {
    ...
    collectOnlyAccount: (name: string) => `Showing ${name}'s collection schedule only.`,
    collectShowAll: "Show all →",
  ...
  PLANNING_TEXT: {
    ...
    divisionToolbarCount: (n: number) => `${n} regions`,
    divisionSelectionNoun: "region(s)",
    divisionClearSelection: "Clear selection",
    divisionBulkRemove: "Remove",
    divisionBulkRemoveTarget: (n: number) => `${n} selected regions`,
    divisionBulkRemoveConsequence: (noun: string) =>
      `The selected regions will be removed - this cannot be undone. Any still holding ${noun} will not be removed.`,
    divisionBulkRemoveDone: (removed: number) => `${removed} region(s) removed`,
    divisionBulkRemoveSkipped: (n: number, noun: string) => `${n} still holding ${noun}, not removed`,
    divisionToolbarFilteredCount: (shown: number, total: number) => `${shown} of ${total} divisions`,
    divisionSearchPlaceholder: "Search by division name or code",
    divisionSearchLabel: "Search",
    divisionResetFilters: "Reset filters",
    divisionSourceFilterLabel: "Filter by source",
    divisionFilterAllSources: "All sources",
    divisionFilterEmpty: "No matching divisions",
  ...
  STRATEGY_TEXT: {
    ...
    colSegmentCriteriaPlan: "Definition · plan",
  ...
  PERMISSION_TREE_TEXT: {
    ...
    pageLabel: {
      ...
      "pipeline.contracttype": "Contract types",
      "pipeline.businessform": "Business forms",
      "pipeline.opportunityconfig": "Business settings",
      "delivery.project": "Projects",
      "delivery.milestone": "Milestones",
      "delivery.revenue": "Collections",
      "delivery.contract": "Contracts",
      ...
      "admin.diagnostics": "System Verification",
      "admin.role": "Roles",
    ...
    actionLabel: {
      ...
      "pipeline.contracttype.view": "View contract types",
      "pipeline.businessform.view": "View business forms",
      "pipeline.opportunityconfig.view": "View business settings",
      "pipeline.opportunityconfig.manage": "Manage business settings",
      "delivery.project.view": "View projects",
      "delivery.contract.view": "View contracts",
      "delivery.contract.upsert": "Record and edit contracts",
      "delivery.contract.renew": "Renew contracts and record outcomes",
      ...
      "admin.audit.view": "View the security audit",
  ...
  PERMISSION_LABEL: {
    ...
    "pipeline.opportunityConfig": "Maintain business settings - contract types, business forms, pipeline stages, win/loss reasons, forecast bands, ageing bands, pricing currency",
  ...
  ADMIN_PAGE_TEXT: {
    ...
    scopeNoTerritory: "No territory assigned",
    scopeCount: (n: number) => `${n} members`,
    pendingMigrationTitle: "Pending migration",
    pendingMigrationWhy: "Pages no navigation or in-app link points to - the code is still here, held for whenever their own feature is decided one way or the other.",
    pendingMigrationCount: (n: number) => `${n}`,
    pendingMigrationEmpty: "No orphaned routes found right now.",
    pendingMigrationAccountComplete: "Batch account completeness - fully built, nothing in the codebase links to it",
  },
  MEMBER_TEXT: {
    title: "Organisation",
    ...
    scopeUnitUnplaced: "Not placed in a unit",
    noun: "member",
  ...
  AUDIT_TEXT: {
    ...
    description: "Records changes to configuration - member and role writes. Never business data. When something is changed, this says who, when, and what.",
    colTime: "Time",
    colActor: "Actor",
    colAction: "Action",
    colObject: "Object",
    ...
    emptyWhy: "Member and role administration writes are recorded here.",
    searchHint: "Search by actor or object",
    ...
    actionLabel: {
      ...
      "admin.member.reactivate": "Reactivate a member",
      "admin.member.scope": "Set data scope",
      "admin.role.upsert": "Create or edit a role",
      "admin.role.remove": "Delete a role",
    } as Record<string, string>,
  },
  ...
  DIAGNOSTICS_TEXT: {
    ...
    description: "Whether this product's own end of the platform connection is healthy. Every probe is read-only except the ones that spend, each called out on its own.",
    refresh: "Re-probe",
    probedAt: (time: string) => `Probed at ${time}`,
    readOnlyNote: "Every probe below is read-only and spends nothing",
    sectionPlatform: "Platform Integration",
    sectionPlatformHint: "Identity (C1), entitlement (C2), usage and provisioning (C3)",
    probe: {
      c1: "C1 - discovery and JWKS",
      tokenMint: "C1 exchange - on-behalf-of token mint",
      c2: "C2 - live entitlement read",
      c3Up: "C3 up - usage consume",
      c3Down: "C3 down - webhook verify and deliveries",
      atlas: "Model plane (Atlas)",
      runos: "Capability plane (Runos)",
      arda: "Shared-data plane (arda)",
    } as Record<string, string>,
    replayTitle: "C3 replay verification (checklist #5)",
    replayHint:
      "One of the probes that spends: reports the same usage row twice and checks the platform replays it by idempotency key instead of double-billing. At most once per workspace per day.",
    replayButton: "Run replay verification",
    replayConfirmTitle: "Run the replay verification?",
    replayConfirmBody: "This sends a real usage row to the platform (only the first click today actually costs anything - a repeat click hits the same idempotency key).",
    replayConfirmAction: "Run it",
    cancel: "Cancel",
    replayRunning: "Running...",
    replayForbidden: "You don't have permission to run this probe - ask another administrator",
    atlasProbeTitle: "Atlas live-call probe",
    atlasProbeHint:
      "One of the probes that spends: sends the shortest possible chat message to Atlas to verify a real call completes end to end. Usage is reported by Atlas itself, on its own metering path - not through this product's usage metric.",
    atlasProbeButton: "Run Atlas probe",
    atlasProbeConfirmTitle: "Run the Atlas probe?",
    atlasProbeConfirmBody: "This makes a real call to Atlas and incurs a real model usage cost.",
    atlasProbeConfirmAction: "Run it",
    atlasProbeRunning: "Running...",
    atlasProbeForbidden: "You don't have permission to run this probe - ask another administrator",
    badgeOk: "OK",
    badgeFail: "Failing",
  ...
  ENABLEMENT_TEXT: {
    title: "Enablement Analysis",
    subtitle: "AI Enablement Analysis",
    home: "Home",
    windowLabel: (days: number) => `Last ${days} days`,
    panelExecuted: "Tasks executed, ranked",
    panelExecutedWhy: "Who is actually executing tasks through the copilot",
    panelAdoption: "Suggestion adoption",
    panelAdoptionWhy: "What the copilot proposed, and who accepted or rejected it",
    colUser: "Member",
    colExecuted: "Executed",
    colAdopted: "Adopted",
    colRejected: "Rejected",
    heroExecuted: "Tasks executed",
    heroAdopted: "Suggestions adopted",
    heroRejected: "Suggestions rejected",
    heroExpired: "Lapsed unactioned",
    heroExpiredHint: 'Workspace-wide only, not attributable to a person - agent_action has no "who this was shown to" column',
    donutTitle: "Where suggestions ended up",
    donutAdopted: "Adopted",
  ...
  DEAL_PAGE_TEXT: {
    ...
    factOwner: "Owner",
    factContractType: "Contract type",
    factBusinessForm: "Business form",
    factSource: "Source",
    sourceCampaign: (name: string) => `Campaign · ${name}`,
    sourceDirect: "Entered directly",
    factTerritory: "Territory",
    factCreated: "Created",
    factBudget: "Customer project budget",
    factTeam: "Presales / delivery",
    notSet: "Not set",
    amountLabel: "Amount",
    amountNone: "Not priced",
    editTerms: "Edit deal record",
    solutionTitle: "Product solution",
    solutionFrom: (name: string) => `From solution "${name}"`,
    solutionCustom: "Custom combination",
    solutionScenario: (s: string) => `For: ${s}`,
    solutionStandard: "Standard",
    solutionOptional: "Add-on",
    solutionCustomisations: "Customisation",
    customOn: (product: string) => `on "${product}"`,
    solutionItems: (n: number) => `${n} item(s)`,
    solutionOptionalCount: (n: number) => `${n} add-on(s)`,
    ...
    decisionTitle: "Decision process",
    decisionEmpty: "No one's role on this deal is stated yet",
    decisionNone: "No one is on this deal's chain yet",
    decisionReachable: (n: number) => `${n} people · economic buyer reachable`,
    decisionUnreachable: (n: number) => `${n} people · economic buyer unreachable`,
    ...
    customerOpenDeals: "Other open deals here",
    dealsCount: (n: number) => `${n}`,
    ...
    progressSummary: (stage: string, days: number | null) =>
      days === null ? stage : `${stage} · ${days} days in stage`,
    probability: (n: number) => `Win rate ${n}%`,
    closeOn: (d: string) => `Close ${d}`,
    plan: "Plan",
    history: "History",
    reasonsTitle: "Reason to buy",
    requirement: "Requirement",
    requirementNone: "No requirement written down yet",
    competitionTitle: "Competition",
    competitionMentions: (n: number) => `${n} mention(s) of a rival in follow-ups`,
    quoteTitle: "Quote and approval",
    ...
    reviewSummaryOwed: (outcome: string) => `${outcome} · review not written`,
    reviewSummaryDone: (outcome: string) => `${outcome} · reviewed`,
  },
};
