...
export const en: Dictionary = {
  ...
  WALLET_TEXT: {
    ...
    fieldHint: (currency: string) => `What the customer spends on this project in total (${currency}), entered by hand, used for the deal share; clear it to unset`,
    ...
    dealNoBudget: "No budget entered",
    dealNotOurs: "Not won - not counted",
    dealUnpriced: "No amount",
    ours: (v: string) => `ours ${v}`,
    budget: (v: string) => `customer total ${v}`,
    by: (name: string, date: string) => `${name}, ${date}`,
    dealDetail: (basis: string, ours: string, budget: string) => `${basis}: ours ${ours} / customer total ${budget}`,
  ...
  DEAL_PAGE_TEXT: {
    ...
    customerTitleHint: (name: string) => `Open the customer: ${name}`,
    customerName: "Name",
    customerIndustryRegion: "Industry · region",
    customerOtherDeals: "Other open deals",
    customerContracts: "Contracts",
    contractsCount: (n: number, active: number) => (n === 0 ? "None" : `${n} · ${active} in force`),
    projectsCount: (n: number, red: number, amber: number) =>
      n === 0 ? "None" : red + amber === 0 ? `${n} · all on track` : `${n} · ${[red ? `${red} red` : "", amber ? `${amber} amber` : ""].filter(Boolean).join(" ")}`,
    ...
    evidenceEmpty: "Not written",
    evidenceGrounded: "Cited",
    evidenceSaid: "Said",
    evidenceAccepted: "From the advisor, confirmed",
    evidenceFill: "Write",
    evidenceEdit: "Revise",
    evidenceHistory: (n: number) => `${n} earlier version(s)`,
    evidenceCleared: "(cleared)",
    evidenceDialog: (slot: string) => `Write down: ${slot}`,
    evidenceDialogWhy: "Every save is a new version; earlier ones stay in the history. Saving it empty clears it.",
    evidenceStatement: "Statement",
    evidenceCite: "Which follow-up it rests on (optional)",
    evidenceCiteNone: "None (said)",
    evidenceSave: "Save this version",
    exitTitle: (met: number, total: number) => `Exit criteria for this stage ${met}/${total}`,
    exitShort: (met: number, total: number) => `exit ${met}/${total}`,
    exitNone: "No exit criteria set for this stage",
    exitMet: "Met",
    exitUnmet: "Not met",
    exitUnknown: "Can't tell",
    exitAnyone: "any contact",
    exitHolders: (n: number) => `${n} people`,
    exitNoRole: (roles: string) => `No ${roles} on this deal yet`,
    exitNeverReached: (roles: string) => `${roles} not in any follow-up on this deal`,
    exitReached: (d: number) => `reached ${d} days ago`,
    exitReachedLate: (d: number, limit: number) => `${d} days ago - over ${limit}`,
    exitWritten: "Written",
    exitGoTo: (panel: string) => `Not written ↗ ${panel}`,
    exitNoLines: "No lines yet ↗ Quote",
    exitPendingLines: (n: number) => `${n} line(s) awaiting approval ↗ Quote`,
    exitPriced: (n: number) => `${n} line(s) priced`,
    exitOverdue: (n: number) => `${n} of theirs overdue · see the plan`,
    exitNoOverdue: "Nothing overdue",
    exitNoDate: "No close date",
    exitDaysLeft: (n: number) => `${n} days left`,
    exitDatePassed: (n: number) => `${n} days past`,
    exitUnreadable: "Can't read the evidence",
    ...
    importanceConfirm: "Confirm",
    importanceSaved: "Importance updated",
    importancePriority: (p: number | null) => (p === null ? "Unranked" : `Priority P${p}`),
    ...
    foldMore: (n: number) => `More to-dos · ${n}`,
  ...
  EVIDENCE_ERROR: {
    ...GATE_ERROR,
    not_found: "No such deal, or it belongs to another workspace",
    evidence_slot_unknown: "There is no such evidence slot",
    evidence_too_long: "A statement is at most 2000 characters",
    evidence_citation_foreign: "A statement can only cite a follow-up on this deal",
  },
  EXIT_CONFIG_TEXT: {
    title: "Stage exit criteria",
    why: "What each stage should have got us - judged by the product from records it holds, not ticked. The deal page checks the current stage's; moving on is not blocked.",
    count: (n: number) => `${n}`,
    add: "Add criterion",
    edit: "Edit",
    remove: "Remove",
    addTitle: "Add an exit criterion",
    editTitle: "Edit the exit criterion",
    save: "Save",
    kindLabel: "Judged by",
    kindLocked: "How a criterion is judged cannot change - remove it and add another, so past checks keep their meaning.",
    kind: {
      role_present: "A role is on the deal",
      role_reached: "A role was reached recently",
      slot_filled: "Evidence is written",
      lines_priced: "Lines are priced",
      their_commitments_clear: "Nothing of theirs overdue",
      close_date_valid: "Close date not past",
    } as Record<string, string>,
    rolesLabel: "Roles",
    rolesAnyone: "None selected = at least one contact on the deal",
    daysLabel: "Within days",
    slotLabel: "Which evidence",
    nameLabel: "The sentence the deal page shows",
    nameHint: 'e.g. "Economic buyer reached within 30 days"',
    describeRolePresent: (who: string) => `${who} on the deal`,
    describeRoleReached: (who: string, days: number) => `${who} reached within ${days} days`,
    describeSlot: (slot: string) => `${slot} written`,
    removeTitle: "{verb} the exit criterion \"{target}\"?",
    removeConsequence: "The deal page stops checking it; past checks already in the stage journal are unaffected.",
  },
  EXIT_CRITERION_ERROR: {
    ...GATE_ERROR,
    not_found: "No such criterion - it may just have been removed; refresh and retry",
    name_required: "Write the sentence the deal page shows",
    name_too_long: "At most 255 characters",
    criterion_kind_unknown: "There is no such way of judging",
    criterion_param_invalid: "Parameters don't fit: pick at least one role (reached), 1-365 days, evidence from the list",
  ...
  DEAL_SCORE_TEXT: {
    title: "Deal score",
    why: "The deal assessment's five dimensions - the dossier's score and 态势判决's five cards both use them: each 0-100, averaged with the weights here. A dimension with nothing to judge is left out and its weight spread over the rest.",
    sum: (n: number) => `Weights total ${n}/100`,
    weightsLabel: "Dimension weights",
    weightsHint: "The five weights must add up to 100. A weight of 0 leaves the dimension out.",
    factor: {
      value: "Need & value",
      consensus: "Buyer consensus",
      competition: "Competitive position",
      engagement: "Engagement",
      progress: "Momentum",
    } as Record<string, string>,
    factorHow: {
      value: "pain, quantified value, budget written; any inaction signal",
      consensus: "economic buyer, coach, opposition, reach, decision process",
      competition: "rivals known, how often mentioned",
      engagement: "days since a touch; their promises overdue",
      progress: "exit criteria, stall, close date, next step, approvals",
    } as Record<string, string>,
    watchLabel: "Watch score",
    watchHint: "What an indicator scores at watch (steady 100, risk 0); a dimension is the mean of its indicators",
    recentLabel: "Recent within (days)",
    recentHint: "Touched within this many days: engagement's contact indicator is steady",
    quietLabel: "Quiet after (days)",
    quietHint: "Untouched this long: risk; in between: watch",
    save: "Save",
    reset: "Reset to defaults",
    formula: "Score = Σ(weight × dimension) ÷ Σ weight",
    saved: "Deal score weights saved",
    coinLabel: "Deal score",
    band: { good: "Healthy", warn: "Watch", bad: "At risk" } as Record<string, string>,
    concern: (factor: string, value: number) => `Biggest loss: ${factor} (${value})`,
    noConcern: "Every dimension at full marks",
    gap: {
      pain: () => "Write down the pain",
      metrics: () => "Quantify the value",
      budget: () => "Find out the budget",
      statusQuo: () => "Inaction signal - address it",
      noEconomic: () => "Find the economic buyer",
      reachEconomic: () => "Get to the economic buyer",
      coach: () => "Develop a coach",
      opposition: (n: number) => `Handle ${n} opponent(s)`,
      coverage: (n: number) => (n === 0 ? "No one on this deal yet" : "Only one contact - widen it"),
      decisionProcess: () => "Learn the decision process",
      rivalMentioned: (n: number) => `Rival mentioned ${n} time(s) - find out who`,
      rivalUnknown: () => "Find out who the rivals are",
      neverTouched: () => "No follow-up yet",
      quiet: (n: number) => `${n} days quiet - reach out`,
      theirOverdue: (n: number) => `They owe ${n} overdue item(s) - chase`,
      exitGap: (n: number) => `${n} exit criteria to go`,
      stalled: (n: number) => `Stuck in stage ${n} days`,
      closePassed: () => "Close date passed - reset it",
      slipped: (n: number) => `Close date pushed ${n} time(s)`,
      nextStep: () => "Book the next step",
      approval: (n: number) => `${n} discount line(s) awaiting approval`,
    } as Record<string, (n: number) => string>,
    allGood: "All in place",
    indicator: {
      pain: "Pain",
      metrics: "Quantified value",
      budget: "Budget",
      statusQuo: "Inaction signal",
      chain: "Buying group",
      economic: "Economic buyer",
      coach: "Coach",
    ...
    verdict: { good: "Steady", warn: "Watch", bad: "Risk", unknown: "Unknown" } as Record<string, string>,
    verdictNone: "None",
    todoCta: "Handle",
    todoReason: (dimension: string, indicator: string) => `${dimension} · ${indicator}`,
    unknownDim: "Nothing to judge yet",
    howTo: "Weights in Admin · Opportunity",
  },
  DEAL_SCORE_ERROR: {
    ...GATE_ERROR,
    weight_out_of_range: "Each weight sits between 0 and 100",
    weights_not_100: "The seven weights must add up to exactly 100",
    watch_out_of_range: "The watch score sits between 1 and 99",
    recent_out_of_range: "Recent runs from 1 to 365 days",
    quiet_not_after_recent: "Quiet must come after recent, within 365 days",
    unknown: "Not saved - try again later",
  },
  JUDGEMENT_ACTION_TEXT: {
    title: "Analysis & judgements",
    listTitle: "Findings & actions",
    listSep: ", ",
    why: {
      pain: () => "The pain is not written: why they would change is unclear",
      metrics: () => "The value is not quantified",
      budget: () => "The customer's project budget is not entered",
      statusQuo: (q: string) => `An inaction signal in the follow-ups: "${q}"`,
      noEconomic: () => "No economic buyer is named on this deal",
      reachEconomic: (name: string, days: number | null) => (days === null ? `${name} never contacted` : `${name} not met for ${days} days`),
      coach: () => "No coach on this deal",
      opposition: (names: string) => `${names} opposed`,
      coverage: (n: number) => (n === 0 ? "No one on this deal yet" : `Only ${n} contact on this deal`),
      decisionProcess: () => "Their decision process is not written",
      rivalMentioned: (text: string) => `Latest rival mention: "${text}"`,
      rivalUnknown: () => "No rival in the follow-ups - which is not the same as none",
      neverTouched: () => "No follow-up on this deal yet",
      quiet: (days: number, date: string) => `Last follow-up ${days} days ago (${date})`,
      theirOverdue: (n: number, max: number) => `They owe ${n} overdue item(s), the oldest ${max} days`,
      exitGap: (names: string) => `Unmet: ${names}`,
      stalled: (stage: string, days: number) => `In ${stage} for ${days} days`,
      closePassed: (date: string) => `Expected close ${date} has passed`,
      slipped: (n: number) => `Close date pushed ${n} time(s)`,
      nextStep: () => "No next step booked - neither side has a promise dated ahead",
      approval: (n: number) => `${n} line(s) below floor awaiting approval`,
    },
    evidence: "Evidence",
    toPlan: "Add to plan",
    ask: "Ask the advisor",
    go: "Handle",
    analyse: "Analyse this deal",
    adopt: "Adopt",
    reanalyse: "Re-analyse",
    ignore: "Ignore",
    cancel: "Cancel",
    adoptTitle: "Adopt into the plan",
    adoptHint: "Write it as a step on our side with a due date. Adopted, it joins the plan and this judgement leaves the list.",
    adoptDraft: (claim: string) => `Follow up: ${claim}`,
    statementLabel: "What to do",
    dueLabel: "Due",
    confirm: "Adopt",
    adopted: "Added to the plan",
    ignored: "Ignored",
    reanalyseQuestion: (deal: string, claim: string) =>
      `Using everything on the deal "${deal}", analyse this judgement further: "${claim}" - what it means, the likely cause, and how to move forward.`,
    empty: "No judgements yet. Ask the advisor to analyse this deal.",
  },
  JUDGEMENT_ADOPT_ERROR: {
    ...GATE_ERROR,
    statement_required: "Write what to do",
    due_invalid: "Pick a due date",
    unknown_direction: "Wrong direction",
    unknown: "Not adopted - try again later",
  },
  IMPORTANCE_ERROR: {
    ...GATE_ERROR,
    not_found: "No such deal, or it belongs to another workspace",
    importance_level_unknown: "That level is not one of this workspace's - refresh and choose again",
    unknown: "Not saved - try again later",
  },
  PLAN_DRAFT_ERROR: {
    ...GATE_ERROR,
