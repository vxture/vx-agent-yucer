...
export type NavIcon =
  ...
  | "tree-structure"
  | "users"
  // 角色管理 uses "user-circle", not "role" (TD-027,
  // docs/60-operations/00-index.md): the DS's icon table has `role` pointing
  // at the same `UsersIcon` as `users` above, so the two nav items drew one
  // glyph under two names. Went through "medal" (collided with 职级),
  // "certificate"/"shield" (vetoed as reserved for a real future feature)
  // and "rows" (too plain, owner asked for something with actual identity)
