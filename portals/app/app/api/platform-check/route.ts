  ...
  try {
    ...
    return {
      ...
      detail:
        ...
        "no counter metric is defined for yucer yet, so nothing is recorded at an action point",
    };
  } catch (err) {
...
const c3Replay: ProbeResult = {
  configured: false,
  ok: false,
  detail:
    "the idempotency probe (checklist #5: same key twice -> replayed:true, same event_id) needs a registered counter metric; yucer has none defined yet",
};
