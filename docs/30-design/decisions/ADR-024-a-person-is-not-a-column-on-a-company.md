...
- 状态：**已接受**（owner 裁定 2026-09-04）
- 日期：2026-09-04
- 落地：A、B 两批 #181；C 批 #182；D 批本次。四批全部完成，无妥协项。
...
**演示数据里这个缺陷已经具体存在**：`acc_demo_2` 名下有供应链协同平台、智能仓储升级、
生产排程系统三条商机记录，共用一条决策链——「决策人未触达」这个徽章在同一客户的每一单
上要么全亮要么全灭，而真实情况几乎不可能一致。

> **2026-09-04 更正（D 批落地时）。** 上一段初稿写的是「七个客户各有 2–3 个在谈商机」，
> 而实际查过种子数据后：`acc_demo_2` 的三条里只有一条**在办**，另两条已赢/已丢，而且
> **当时没有任何一个客户同时有两个在办商机**。也就是说这个缺陷当时在演示数据上根本
> **演示不出来**，草案却断言它「已经具体存在」。缺陷本身的论证不依赖那句话——角色按单
> 而非按人，是方法论的定义——但用一个没核对过的数据描述去支撑它是不对的。
> D 批为此在 `acc_demo_2` 上补了第二条在办商机（`opp_demo_21` 生产排程系统），
> 并给两单写入不同的采购角色，缺陷与修复现在都真的看得见。
...
> **2026-09-04，D 批落地时的记录。** 这条差点没有被贯彻。D 批的初稿把
> `person.decision_role` 留成了"客户级默认"，让没有 `opportunity_contact` 行的商机回落到
> 它——理由是"删掉会销毁没有在办商机的客户上已录入的角色"。owner 指出**产品还没有任何
> 用户**，库里每一行都是本仓自己写的种子数据，那条理由因此不成立：它保护的是一份不存在的
> 数据，代价却正是本条反对的形状。回落被删除，两列一并移除，`opportunity_contact` 是
> 采购角色的唯一来源。
>
> 没有在办商机的客户**没有采购委员会**——这是一句真话，而旧的那一列是一个带着 schema 的
> 猜测。那样的客户有的是人和职务，那是 `person_affiliation` 回答的另一个问题。
