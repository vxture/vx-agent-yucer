...
**2026-08-29 补记 —— 「可机器校验」原来从未被校验，一查就破。**

登记写着非 ASCII 只在三个文件、「可用一行命令机器校验」。**那行命令从未存在**，
真跑一遍：粗计 **30 个文件**含非 ASCII。与 TD-010 的「复发两次实为 21 处」同病：
没人重新量过的登记会悄悄脱离它描述的代码库。

精确量（只看**字符串字面量**、剥掉注释）后拆成三类：27 个文件是**英文注释引用中文**
（解释设计时引用产品文案或上游文档原文——给维护者读的文档，不是产品文案）；两处是
排版分隔符 `·`；**一处真散落**：`demo-seed.ts` 内联了两个细分市场名——恰好违反本登记
自己写的「demo-seed 只引用 fixtures、自身不含中文」。已归位进 `DEMO_SEGMENTS`。

守卫 `ascii-containment.test.ts` 把该守的那半句变成机器问题：**含 CJK 的字符串字面量
只许出现在四个点名文件里**（判断句文件 `judgement.ts` 在列）。注释不扫——那是文档；
排版符不算——那不是文本。债本身（这些文件该不该存在）仍是平台仓的裁定，守卫只保证
收敛不再无声腐烂。反证两条：种子里塞回内联名、组件里塞一个中文字面量，都被带行号点名。
...
**为什么当时先记不修**：定时器在平台侧且唯一，暴露面是运维手动重复触发，不是
系统自己制造的竞态。**不用应用层加锁顶替**——两个副本下那个锁是错的，会给出一种
已经解决了的错觉。

**2026-08-29 已还**，按上面写好的修法原样：增量 `0016_sweep_unique_index.sql`
落那条部分唯一索引；Prisma 端把 `P2002` 映射为静默跳过（输掉竞态不是故障——
对方的提案已经在队列里，那正是调用方要的结果）；内存实现同语义（含 NULL 键不
约束）；`sweep` 的台账把被地板吸收的写计入 `alreadyQueued`，而不是丢在两个计数
器之间。测试三条 + 数据库直连一条（两次同键插入第二次违反、拒绝后可重提、NULL
键两次都落）；反证：拿掉内存地板 → 「the floor must absorb the duplicate」红。
...
**2026-08-29 本仓一半已还，三件：**

1. **版本进审计**。`ToolInvocation` 增 `versionResolved`，invoke 返回的
   `meta.version_resolved` 此前被原地丢弃——「昨天好好的今天不对了」从此能查到
   两次调用各自跑的是哪个版本。
2. **turn 内钉住**。同一 turn 里对同一能力的第二次调用，显式携带第一次被服务的
   版本。`stable` 是浮动别名，运营者中途改指向，此前同一次对话的前后两问可以落在
   不同版本上且无从发现。**第一次调用刻意浮动**——钉的是一个 turn，不是产品；
   下一个 turn 重新浮动。网关不报版本时保持浮动而不是钉一个编造值。
3. **弃用告警**。`resolve()` 读已有的 `state` 字段，命中 deprecated 时告警。
   不编造上游 schema：生命周期就在 `state` 里，此前没人读。

反证两条：去掉钉住 → 「第二次调用必须携带第一次的版本」红；网关不报版本时硬钉
`stable` → 浮动测试红。

**仍开的另一半**：跨 turn / 跨会话的钉住是持久化配置设计（钉在哪、谁解钉）；
Atlas 模型面**根本没有版本列**是平台侧的债，本仓修不了。
...
刻意不在必需检查里**，所以不阻塞合并：把一个**第三方服务**折进必需检查，等于让它
宕机时全仓无法合并。

> **这里原来把 `sonar` 和 `db-contract` 说成「同一个理由」，那个等价是错的**
> （已于 2026-09-01 更正）。`db-contract` 不是第三方服务——它是同一个 runner 里的
> 一个 Postgres service 容器，没有外部依赖、没有配额、没有别人的可用性。它当天已
> 加入必需检查，成为第六项。真正的区别不是「会不会挂」，而是**挂了算谁的**：
> SonarCloud 宕机与本仓的代码无关，`postgres:18` 起不来与本仓的 DDL 有关。
...
08-29 那班截至 12:09 UTC 未见。

**已结清 2026-08-30，而且上面那句「应跑未跑」是我判早了。**

```
2026-08-29T12:41  schedule  success   (11 秒，无告警 → 主干干净)
```

它跑了。**我 12:09 UTC 查的，它 12:41 UTC 落地——早查了 32 分钟就下了结论。**
教训不在机制，在核查方法：判「计划任务响没响」必须把 GitHub 的调度迟延算进去，
而这里的迟延是**小时级不是分钟级**。

**真正该记的是迟延本身：cron 写 06:17 UTC，实际 12:41 UTC，迟 6 小时 24 分。**
工作流注释说选非整点分钟是为了避开 GitHub 的 :00 高峰——从这次看，那没多大用。
对一个日频公告巡检这是可接受的（公告不是分钟级的，TD-012 的失败是十天不是六小时），
但**「每天 06:17」是虚的，真实语义是「每天，某个时候」**。谁按 cron 字面去对时间都会对错。

08-30 那班截至 08:23 UTC 仍未出现（已过 2 小时 6 分），与 08-29 的迟延量级一致，
不构成新的失效证据。

告警的抬起与撤销能力已于 08-28 用 issue #67 自测证明过（建了又关）。本项结清：
**这个机制活着，只是不准时。**
...
**本地按 CI 的步骤原样跑过一遍验证**（不是推上去碰运气）——合并前后各文件的行
覆盖率：
...
`SF:`/`end_of_record` 配平，路径从仓库根全部可解析，无文件丢失。

**上线后实测，以及一处必须记下来的预测偏差。** 合并落到 main（`47ace361`）后，
SonarCloud 的项目级覆盖率从 **89.4% 升到 92.4%**（前一次分析 `46df6965` 是
TD-020 的纯文档 PR，两次之间唯一影响覆盖率输入的就是本次合并）。

本地replay 当时算出的是「91.83% → 95.63%」，**和 Sonar 实际报的数不是同一个数，
当时把它写成了「Sonar 读到的」，这是错的**：本地那个比值的分母只有 lcov 里出现过
的文件，而 Sonar 的分母是全部可分析行（`lines_to_cover` 28098），**包括那些从未
出现在任何 lcov 里、因此整份算作未覆盖的文件**。方向和幅度对得上（+3.8 对 +3.0），
绝对值不对得上，因为两者根本不是同一个比值。

留着这段不是自我检讨，是这条 TD 的同一课在更小的尺度上又出现了一次：**一个没说
清分母的百分比，即使算得对，也会被读成另一个数。** 引用本仓覆盖率时，除了说明是
哪一半（离线／db／合并），还要说明是哪个分母（lcov 内文件／Sonar 全量可分析行）。
...
没有 stepper / timeline / 步骤条这一族。最接近的 `LevelMarker` 是 L1-L5 的等级底座,
它的文档明确写着那套渐变材质"除了它以外的地方不该出现",不能借用。

**缺失元素**:一个有序步骤条——节点按序排列、节点间有连接线、节点状态可着色(完成 /
失败 / 未开始),支持横向滚动。**垫片位置**:
`(app)/components/delivery-plan-flow.tsx`,用 DS 的 `Icon` 与意图色令牌
(`--success-border` / `--danger-surface` / `--border` 等)组合,不改 DS 样式、不重写
任何 DS 元件。**回收条件**:DS 提供步骤条后,把这个文件换成对它的薄封装(注入里程碑
...
### TD-028 - 已应用的增量被原地修改，没有任何守卫

2026-09-14 在生产 twin 上演练 db-init 时发现：`incr/0051_org_structure.sql`
在生产于 09-10 应用之后，被 PR #244 原地改了根节点代码（`hq` ->
`headquarters`），同一 PR 同步改了 `lib/org.ts` 的镜像。账本（ADR-032）每个
文件只应用一次，生产模板表因此停在旧值，「表与镜像一致」的 db 测试对生产形态
的库必败；`incr/0069` 补上了那次改动本该有的增量。

**缺口**：CI 里没有任何东西比较一个增量与它已经上线的版本——db-contract 在
空库上按 HEAD 顺序全跑，改过的增量和没改过的一样绿。**收回条件**：一条守卫，
...
**同一处的第二个垫片（2026-09-23）**：DS 标题行是 `items-start`，图标带 `mt-2xs` 下移、操作区
钉在底部（`self-end`）——没有副标题时图标与标题错位，按钮位置随标题高度变化。
`HEADER_ALIGN_CLASS` 让图标、标题块、按钮共用一条中轴（单行时一行居中，两行时跨两行居中），
同样依赖 DS 内部结构。
