...
TD-002 的现状是**收敛而非解决**：全部面向用户的字符串集中在
`portals/app/app/(app)/lib/messages.ts` 一个文件，它是 `app/` 下唯一含非 ASCII 的
源文件，其余全部可机器校验。标准修订后二选一收口——平台仓为终端用户文案开显式口子，
或引入正式 i18n 方案把文案移出源码树。**在裁定之前收敛状态维持不变，不要「顺手修掉」**：
把文案改成 `\uXXXX` 转义确实能满足字面规则，但会得到一份人类无法维护的文案。
