...
两个数，**不合成一个**（owner，2026-09-23）：

- **年化合同额** = 今天生效的合同，各自按自己的期限折算到一年（总额 × 365 ÷ 期限天数）。
  **不叫 ARR**：产品目录没有区分订阅与一次性，生效中的一次性合同也会被年化，标签不能说出
  数据不知道的事。
- **历史合同总额** = 全部已签（非草稿）合同的总额，不论是否仍生效。

「生效」与 §9.2 已购态同一口径（状态 + 期限）。按币种分行，**不跨币种相加**。已签但未填
金额的合同**计数说明、不计入**——当 0 计会读成「一文不值」。派生不落表：每次读时由
`installedRevenue()`（`domains/delivery/lib/contract.ts`）算出。

### 9.6 钱包份额

钱包份额 = **我方承接额 ÷ 客户在这个项目上的总投入**，按**商机**算（owner，2026-09-24）。
不按客户整体算：「客户在我们这一类上总共花多少」没有谁说得清边界，而一个商机所属项目的
总预算是售前本来就要问的问题。

- **分母**：商机上的「客户项目总投入」（incr/0080），**人工填报**，商机币种；谁填的、何时填的
  由服务端随写入记下，页面照实显示，旧数字读起来就是旧的。没填就是没有，**不当 0**。
- **分子**：赢单的商机 = 这个商机名下已签（非草稿）合同的总额（同币种）；还没有合同或合同未填
  金额时退回商机金额。在谈的商机 = 商机金额。输掉、放弃的商机不计。
- **汇总**（单位卡、存量收入卡）只做加总，两个数**不合成一个**：**已承接**（赢单）与**在谈**
  （开放）。只加填了总投入的商机，并说明「N 个里 M 个已填」；按币种分行，**不跨币种相加**。
- 我方承接额超过客户总投入时照实显示比例并提示「总投入可能填低了」，不截到 100%。

派生不落表：`walletShare()`（`domains/pipeline/lib/wallet-share.ts`）每次读时算出。
