...
## 2026-09-20 增量 - 关联协作人（incr/0074）

权限 26 → 27，授权 420 → 442。角色数不变，仍是 31。

| 权限 | 是什么 | 授予 |
|------|--------|------|
| `account.collaborator` | 搜索工作区同事并把TA加为这个客户的协作人（增/删名单，主负责人不变） | 与 `account.record` 完全相同的 22 个角色 |

**不是 `account.write`。** 搜索同事姓名要读工作区成员名录，而这个仓库目前唯一的
成员名录读接口（`listWorkspaceMembers`，`authz/admin.ts`）是特意做成仅管理员可读
的——它撑的是 `/admin` 付费席位统计，不是给销售搜同事用的。复用它等于把一个管理员
能力发给每一个能编辑客户的人；这条权限只做它自己要做的事：搜索候选人、增删协作人
名单。

**读现有协作人名单不占这条权限，走 `account.view`。** 谁已经在协作人名单里，是
这份客户档案本来就该有的一条事实，跟行业、区域一样——真正新增的能力只有"搜索并
增删"这一半，只有它需要新权限。

**只授予写，没有独立的只读版本。** `authz/actions.test.ts` 有一条硬规则：只读
成员（`viewer`）必须持有系统里每一个只读动作的权限。这条权限如果拆出一个
`account.collaborator.view`，`viewer` 就必须也拿到它——但没人要求一个只读成员
能搜工作区同事名录。所以搜索候选人的读，跟增删一样，都挂在 `account.collaborator`
这一个写权限下：搜索是"管理协作人"这个动作里的一步，不是它自己的用例。

**授予的 22 个角色跟 `account.record` 完全同一批**：一起经营一个客户关系的人，
跟记录这个关系里发生了什么的人，是同一群人。不给 `sales_ops`/`viewer`（跟
`account.record` 一样的排除理由），也不给 `executive`/`finance`/
`workspace_admin`/`deal_desk`/`ops_head` 等纯管理向角色——这些角色从一开始就没有
`account.record`。
