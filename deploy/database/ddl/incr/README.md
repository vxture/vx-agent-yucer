...
| `0068_restore_grants_after_failed_rerun.sql` | Grants only. Production's failed second db-init (2026-09-10, pre-ledger) re-ran 97 (DELETE reached five SELECT+INSERT tables) and half of 98 (the `account` REVOKE ran, the re-GRANT died on `industry`; member lost 0022's `scope`). Found on a production twin that replayed the failed run; a no-op restatement on a fresh database. |
| `0069_root_unit_code_headquarters.sql` | PR #244 renamed the templates' root unit `hq` -> `headquarters` by editing 0051 in place after production had applied it. This is the increment that edit should have been: a guarded rename in the template table and in seeded workspace trees; links are by id. |
